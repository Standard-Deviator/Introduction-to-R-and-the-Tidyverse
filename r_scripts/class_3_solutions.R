...
eye_color_3 <- fct_lump(f = superhero_data$`Eye color`, n = 3)
...
eye_color_05 <- fct_lump(f = superhero_data$`Eye color`, prop = 0.05)
...
left_join(class_list,class_grades, by = "id")
...
left_join(class_grades,class_list, by = "id") %>%
  select(id, name, grade)


# Only keep rows that have a match in both tabes
inner_join(class_grades,class_list, by = "id")


# merge everything and insert NA where applicable
full_join(class_grades,class_list, by = "id")
