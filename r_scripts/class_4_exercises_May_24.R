...
graph_data <- superhero_data %>%
    filter(Height < 500, !is.na(Height), !is.na(Gender))
...
# turn this scatterplot into a series of scatterplots using Alignment for the columns and Gender for the rows
# How else can we clean up this plot
graph_hist_data <- superhero_data %>%
...
ggplot(data = graph_hist_data, mapping = aes(x = Weight, y = Height)) +
